var plugin = (function () {
    ...
    //  сюди Project URL та anon public key - тоді плагін буде передзаповнений
    ...
    function lang () {
      Lampa.Lang.add({
        ...
        dblink_settings_supa_key: {
          ...
          uk: 'Supabase anon key',
          ru: 'Supabase anon key'
        },
        dblink_settings_supa_key_desc: {
          en: 'Public anon key (Settings -> API)',
          uk: 'Публічний anon key (Settings -> API)',
          ru: 'Публичный anon key (Settings -> API)'
        },
        dblink_settings_account: {
          en: 'Sync account',
          uk: 'Акаунт синхронізації',
          ru: 'Аккаунт синхронизации'
        },
        dblink_settings_account_desc: {
          en: 'Separates data of different people in one database',
          uk: 'Розділяє дані різних людей в одній базі',
          ru: 'Разделяет данные разных людей в одной базе'
        },
    ...
    // ═══════════════════════════════════════════════════════════════════
    //  DBLinkClient - фасад над Supabase (PostgREST + Realtime).
    //  Публічний API повністю сумісний зі старим GramLinkClient, тому
    //  верхній шар (Profiles / Hub / Broadcast / дельти) не змінюється.
    //  Маршрутизація "топіків" -> таблиць:
    //    'profiles'      -> таблиця profiles (снапшоти)
    //    'profiles-sync' -> таблиця deltas   (інкрементальні зміни)
    //    'backup'        -> таблиця backups  (повні копії)
    //    heartbeat       -> таблиця devices  (presence)
    //    broadcast       -> таблиця broadcasts (open_card / play_video)
    // ═══════════════════════════════════════════════════════════════════

    // Крихітна одноразова функція-бутстрап. Її треба вставити в SQL Editor
    // ОДИН раз (anon key не має прав на CREATE FUNCTION). Після цього плагін
    // сам створює й оновлює всі таблиці через RPC exec_sql.
    var BOOTSTRAP_SQL = [
    ...
    // Повна схема БД (вшита в плагін). Виконується автоматично через exec_sql.
    // Ідемпотентна: create table if not exists + guard на publication.
    var SCHEMA_SQL = [
    ...
    function DBLinkClient() {
      ...
      this._bcastCursor = null; // ISO рядок останнього побаченого broadcast
      this._ws = null;
      this._wsHb = null;
    }
    ...
    // ─── Креденшали (URL / anon key / account) ──────────────────────────
    ...
    // Сумісність зі старим кодом авторизації (більше не використовується)
    ...
    // ─── Низькорівневий REST-запит до PostgREST ─────────────────────────
    DBLinkClient.prototype._rest = function (method, table, opts) {
      ...
      // Таймаут із налаштувань (dblink_sync_timeout), дефолт 10с
    ...
    // Авто-провіження схеми через RPC exec_sql (потребує одноразового BOOTSTRAP_SQL).
    // Повертає Promise<boolean> - чи вдалось створити таблиці.
    DBLinkClient.prototype.provisionSchema = function () {
      return this._rest('POST', 'rpc/exec_sql', {
        prefer: 'return=minimal',
        body: { sql: SCHEMA_SQL }
      }).then(function (res) {
        return !!res.ok; // ok лише якщо exec_sql існує і SQL виконався
      });
    };
    ...
    DBLinkClient.prototype.connect = function () {
      ...
      // Пінг: читаємо один рядок із profiles
      return self._rest('GET', 'profiles', { query: 'select=id&limit=1' }).then(function (res) {
        ...
        if (self._tablesMissing(res)) {
          // Таблиць немає - пробуємо створити автоматично через exec_sql
          return self.provisionSchema().then(function (ok) {
            ...
            // Перевіряємо ще раз після провіження
    ...
    // ─── Канал/топіки: no-op, повертають сентінели (імена) ──────────────
    ...
    // ─── Профілі / бекапи: sendFile -> INSERT ───────────────────────────
    ...
    // Формує об'єкт повідомлення з рядка profiles/backups
    ...
    // editMessage лишається для сумісності (у поточному коді не використовується)
    DBLinkClient.prototype.editMessage = function () { return Promise.resolve(true); };

    // ─── Дельти: publishRaw / publishDelta / getMessagesSince ───────────
    ...
    // ─── Broadcast (open_card / play_video) ─────────────────────────────
    ...
    DBLinkClient.prototype._startBroadcastPoll = function () {
      ...
      // Стартовий курсор - зараз, щоб не програвати старі команди
      ...
      var tick = function () {
        ...
        self._rest('GET', 'broadcasts', {
          ...
          res.data.forEach(function (row) {
            ...
            var env = row.payload || {};
            var meta = env.meta || {};
            if (meta.device_id === myId) return; // не свої команди
    ...
    // ─── Realtime (best-effort "поштовх" до опитувань) ──────────────────
      ...
      try {
        ...
        ws.onmessage = function (evt) {
          ...
          if (tbl === 'deltas') {
            ...
            // Наступний тік broadcast-поллера підхопить запис миттєво
          }
        };
        ws.onerror = function () {};
        ws.onclose = function () {
     ...
     * @param {string} [options.className='dblink-avatar']
    ...
    // ─── Merge local + Cub plugins (DBLink safeguard) ──────────────
    ...
    function mergeWithLocalPlugins(cubPlugins) {
      ...
      // Keep ALL local plugins first - DBLink lives here
    ...
    function finishMigration(count, activatedId, activatedName) {
      ...
      // Restore dblink_tools with correct onBack instead of Settings.update()
      var enabledCtrl = Lampa.Controller.enabled().name;
    ...
    function applyProfileData(data, msgId) {
      ...
      if (data.plugins) {
        ...
        // 3. Safeguard: DBLink must always be present
    ...
    // ─── Delta apply (called from hub.js event + DBLink.js polling) ──
    ...
    /**
     * plugin_manager.js - DBLink Plugin Manager
     *
    ...
    function initSettings() {
      ...
      SettingsApi.addParam({
        component: 'dblink',
        param: {
          name: 'dblink_open_auth',
          type: 'button'
        },
        field: {
          name: Lampa.Lang.translate('dblink_settings_section_auth')
        },
        onRender: function onRender(item) {
          var client = DBLinkClient.getInstance();
          if (client.hasCredentials()) {
            var userName = Lampa.Storage.get('dblink_user_name', '');
            item.find('.settings-param__name').text(Lampa.Lang.translate('dblink_settings_auth_logout') + (userName ? ': ' + userName : ''));
          } else {
            item.find('.settings-param__name').text(Lampa.Lang.translate('dblink_settings_section_auth'));
          }
        },
        onChange: function onChange() {
          ...
          if (client.hasCredentials()) {
            Lampa.Modal.open({
              title: Lampa.Lang.translate('dblink_settings_logout'),
              html: $('<div style="padding: 1em">' + Lampa.Lang.translate('dblink_settings_logout_confirm') + '</div>'),
              buttons: [{
                name: Lampa.Lang.translate('dblink_settings_logout'),
                onSelect: function onSelect() {
                  client.logout();
                  Lampa.Storage.set('dblink_user_name', '');
                  Lampa.Modal.close();
      ...
      SettingsApi.addParam({
        ...
        field: {
          name: Lampa.Lang.translate('dblink_settings_account'),
          description: Lampa.Lang.translate('dblink_settings_account_desc')
        },
        onChange: function onChange(value) {
          Lampa.Storage.set('dblink_account', String(value || 'default').trim() || 'default');
        }
      });
      SettingsApi.addParam({
        component: 'dblink_connection',
     ...
     *
     * Uses DBLink's existing heartbeat system (getOnlineDevices) to discover
     ...
     *
     * @param {string} key
     * @returns {string} 'core' | 'device_state' | 'cache_exclude' | 'dblink_meta'
     */
    function categorize(key) {
      // DBLink keys are not backed up (except heartbeat/broadcast - device_state)
              ...
              select({
                ...
                onSelect: function onSelect(confItem) {
                  ...
                  downloadManifest(c, session).then(function (manifest) {
                    ...
                    // Safety snapshot (simple: save dblink keys)
    ...
    function startPlugin() {
      ...
      // ── Auto-detect DBLink's own URL for safeguard ──────────
    ...
    function startDeltaPolling() {
      ...
      // Realtime "поштовх": одразу оновити дельти при INSERT у deltas
    ...
    // Витягує ref проєкту з URL (https://<ref>.supabase.co)
    ...
    // Вікно одноразового налаштування: показує BOOTSTRAP_SQL + лінк на SQL Editor.
    // Після цього плагін сам створює всі таблиці через exec_sql.
    ...
    function addProfileHeadButton() {
      ...
      // Profile icons should be visible even when DBLink hub is active
      Lampa.Listener.follow('activity', function (e) {
